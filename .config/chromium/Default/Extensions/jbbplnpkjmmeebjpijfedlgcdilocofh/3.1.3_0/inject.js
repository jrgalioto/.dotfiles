//ONLY EDIT IN EXTENSION_SOURCE DIRECTORY, THIS FILE IS COPIED IN THE BUILD PROCESS AND
//ANY CHANGES WILL BE OVERWRITTEN IF THE EDITS ARE NOT DONE ON THE FILE IN THE EXTENSION_SOURCE DIRECTORY.
var inject = {
    vars: {
        JSDetailsArr: [],
        CSSDetailsArr: []
...
inject.func.setDetailsArray = function () {
        inject.vars.JSDetailsArr = [
            { file: chrome.extension.getURL('wave.min.js') }
        ];
}

inject.func.setCSSArray = function () {
    inject.vars.CSSDetailsArr = [
        chrome.extension.getURL('styles/report-ext.css')
    ];
}
